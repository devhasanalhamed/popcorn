  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<SeriesDetailsBloc>()
        ...
        ..add(GetSeriesRecommnedationsEvent(id)),
      child: Scaffold(
...
class SeriesDetailsContent extends StatelessWidget {
  const SeriesDetailsContent({super.key});
      ...
      builder: (context, state) {
        switch (state.seriesDetailsState) {
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(
                                    (state.seriesDetails!.voteAverage / 2)
                                        .toStringAsFixed(1),
                                    style: const TextStyle(
                                      fontSize: 16.0,
                                      fontWeight: FontWeight.w500,
                              ...
                              Text(
                                _showDuration(
                                    state.seriesDetails!.numberOfSeasons),
                                style: const TextStyle(
                          ...
                          const SizedBox(height: 8.0),
                          Text(
                            '${AppStrings.genres}: ${_showGenres(state.seriesDetails!.genres)}',
                            style: const TextStyle(
                              color: Colors.white70,
                              fontSize: 12.0,
                              fontWeight: FontWeight.w500,
          ...
          case RequestState.error:
            return Center(
              child: Text(state.seriesDetailsMessage),
            );
        }
      },
    );
...
Widget _showRecommendations() {
  return BlocBuilder<SeriesDetailsBloc, SeriesDetailsState>(
      builder: (context, state) {
    switch (state.seriesRecommendationsState) {
      case RequestState.loading:
        return SliverList(
      ...
      case RequestState.done:
        return SliverGrid(
          delegate: SliverChildBuilderDelegate(
            (context, index) {
              final recommendation = state.seriesRecommendations[index];
              return FadeInUp(
                from: 20,
                duration: const Duration(milliseconds: 500),
                child: ClipRRect(
            ...
            childCount: state.seriesRecommendations.length,
          ),
      ...
      case RequestState.error:
        return Center(
          child: Text(state.seriesRecommendationsMessage),
        );
    }
  });
}
